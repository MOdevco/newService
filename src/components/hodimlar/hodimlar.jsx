...
const Hodimlar = () => {
    ...
    const valid = (e) =>{
        if(!dataAuth.password.length && dataAuth.username.length && dataFace.birthday.length && dataFace.firstname.length && dataFace.lastname.length && dataFace.middlename.length && dataFace.passport.length && dataFace.tel1.length && dataFace.tel2.length && dataVal.endDate.length && dataVal.startDate.length && dataVal.stuffId.length == ""){
            
            if(!dataFace.firstname.length == ""){
                setValiDate(false)
            }
            if(!dataFace.lastname.length == ""){
                setValiDate(false)
            }
            if(!dataFace.birthday.length == ""){
                setValiDate(false)
            }
            if(!dataFace.middlename.length == ""){
                setValiDate(false)
            }
            if(!dataFace.passport.length == ""){
                setValiDate(false)
            }
            if(!dataFace.tel1.length == ""){
                setValiDate(false)
            }
            if(!dataFace.tel2.length == ""){
                setValiDate(false)
            }
            if(!dataAuth.password.length == ""){
                setValiDate(false)
            }
            if(!dataAuth.username.length == ""){
                setValiDate(false)
            }
            if(!dataVal.endDate.length == ""){
                setValiDate(false)
            }
            if(!dataVal.startDate.length == ""){
                setValiDate(false)
            }
            if(!dataVal.stuffId.length == ""){
                setValiDate(false)
            }
        }else{
            setValiDate(true)
                            ...
                            <Box display={'flex'} justifyContent={'space-between'} flexDirection={'column'} alignItems={'flex-start'} width={'100%'} gap={'20px'}>
                                <Box display={'flex'} gap={'10px'}>
                                    <FormControl isRequired>
                                        ...
                                        <Input isInvalid={!dataFace.firstname.length ? true : false} onChange={(e) => setDataFace({ ...dataFace , firstname: e.target.value  })} value={dataFace.firstname} width={'300px'}  placeholder='Ismi..' />
                                    </FormControl>

                                    <FormControl isRequired>
                                        <FormLabel>Familiyasi</FormLabel>
                                        <Input isInvalid={!dataFace.lastname.length ? true : false} required onChange={(e) => setDataFace({ ...dataFace, lastname: e.target.value })} value={dataFace.lastname} width={'300px'} placeholder='Familiyasi..' />
                                    </FormControl>

                                    <FormControl isRequired>
                                        <FormLabel>Sharfi</FormLabel>
                                        <Input isInvalid={!dataFace.middlename.length ? true : false} required onChange={(e) => setDataFace({ ...dataFace, middlename: e.target.value })} value={dataFace.middlename} width={'300px'} type='text' placeholder='Sharfi..' />
                                    </FormControl>

                                    <FormControl isRequired>
                                        <FormLabel>Passport</FormLabel>
                                        <Input isInvalid={!dataFace.passport.length ? true : false} required onChange={(e) => setDataFace({ ...dataFace, passport: e.target.value })} value={dataFace.passport} width={'300px'}  placeholder='Passport..' />
                                    </FormControl>

                                    <FormControl isRequired>
                                        <FormLabel>Asosiy telefon</FormLabel>
                                        <Input isInvalid={!dataFace.tel1.length ? true : false} onChange={(e) => setDataFace({ ...dataFace , tel1: e.target.value })} value={dataFace.tel1} width={'300px'}  placeholder='Asosiy telefon..' />
                                    </FormControl>
                                </Box>
                                ...
                                <Box  display={'flex'}gap={'10px'}>
                                    <FormControl isRequired>
                                        ...
                                        <Input isInvalid={!dataFace.tel2.length ? true : false} onChange={(e) => setDataFace({ ...dataFace , tel2: e.target.value })} value={dataFace.tel2} width={'300px'}  placeholder='Qo’shimcha telefon..' />
                                    </FormControl>
                                    ...
                                    <FormControl isRequired>
                                        ...
                                        <Input isInvalid={!dataVal.startDate.length ? true : false} type='text' onChange={(e) => setDataVal({ ...dataVal, startDate: e.target.value })} value={dataVal.startDate} width={'300px'}  placeholder='Ish boshlash sanasi..' />
                                    </FormControl>

                                    <FormControl isRequired>
                                        <FormLabel>Ish yakunlash sanasi</FormLabel>
                                        <Input isInvalid={!dataVal.endDate.length ? true : false}  type='text' onChange={(e) => setDataVal({ ...dataVal, endDate: e.target.value })} value={dataVal.endDate} width={'300px'}  placeholder='Ish yakunlash sanasi..' />
                                    </FormControl>
                                    <FormControl isRequired>
                                        <FormLabel>Tug'ilgan sanasi</FormLabel>
                                        <Input isInvalid={!dataFace.birthday.length ? true : false} onChange={(e) => setDataFace({ ...dataFace, birthday: e.target.value })} value={dataFace.birthday} width={'300px'}  placeholder='Tugilgan sanasi' />
                                    </FormControl>
                                </Box>
                                ...
                                <Box display={'flex'}>
                                    <FormControl isRequired>
                                        ...
                                        <Input isInvalid={!dataAuth.username.length ? true : false} onChange={(e) => setDataAuth({ ...dataAuth , username: e.target.value })} value={dataAuth.username} width={'300px'}  placeholder='Username..' />
                                    </FormControl>

                                    <FormControl isRequired>
                                        <FormLabel>Password</FormLabel>
                                        <Input isInvalid={!dataAuth.password.length ? true : false} onChange={(e) => setDataAuth({ ...dataAuth , password: e.target.value })} value={dataAuth.password} width={'300px'}  placeholder='Password..' />
                                    </FormControl>
                                    <Button
